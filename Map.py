    ...
    def compute_detection_map(self) -> np.array:
        ...
        for i in tqdm(range(self.height), desc="Computing detection map"):
            for j in range(self.width):
                ...
                # Calculate detection level for each radar and take maximum
                max_detection = 0.0
                for radar in self.radars:
                    detection = radar.compute_detection_level(lat, lon)
                    max_detection = max(max_detection, detection)
                    
                detection_map[i, j] = max_detection
        
        # Scale the detection map to [ε, 1] using equation (8) from the problem statement
        if np.max(detection_map) - np.min(detection_map) > 0:  # Avoid division by zero
            detection_map = (detection_map - np.min(detection_map)) / (np.max(detection_map) - np.min(detection_map)) * (1 - EPSILON) + EPSILON
        else:
